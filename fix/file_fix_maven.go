...
func (p *mavenParams) modifyPom(params FixParams) (err error) {
	...
	for _, model := range p.fixModelList {
		...
		for scanner.Scan() {
			...
			if model.Line == line {
				fixText = strings.ReplaceAll(text, params.CompVersion, params.MinFixVersion)
				for i := 0; i < 5; i++ {
					line++
					if scanner.Scan() {
						text2 := scanner.Text()
...
func (p *mavenParams) getFixModelList(params FixParams, pomPathList []string) {
	for _, pomPath := range pomPathList {
		list := GetFixModelList(filepath.Join(params.Dir, pomPath), params.CompName, params.CompVersion, params.MinFixVersion, p.propertyMap)
...
func (p *mavenParams) getExtensionFix(params FixParams, pomPathList []string) {
	...
	split := strings.Split(params.CompName, ":")
	if len(split) != 2 {
		return
	}
	...
	for _, pomPath := range pomPathList {
		list := GetExtensionFixModelList(filepath.Join(params.Dir, pomPath), params.CompName, params.CompVersion, params.MinFixVersion, p.propertyMap)
		p.fixModelList = append(p.fixModelList, list...)
...
func (p *mavenParams) getInheritFix(params FixParams, pomPathList []string) {
	...
	for _, pomPath := range pomPathList {
		list := GetInheritFixModelList(filepath.Join(params.Dir, pomPath), params.CompName, params.CompVersion, params.MinFixVersion, p.propertyMap)
		p.fixModelList = append(p.fixModelList, list...)
